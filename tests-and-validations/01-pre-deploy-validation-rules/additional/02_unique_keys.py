class Rule:
    ...
    @classmethod
    def _identify(cls, item, fallback_index):
        # Identify by the item's first key/value pair
        if isinstance(item, dict) and item:
            key, value = next(iter(item.items()))
            return "{}={}".format(key, value)
        return "[{}]".format(fallback_index)
    ...
    def match_path(cls, inventory, full_path, search_path, parent_context=None):
        ...
        for idx, path_element in enumerate(path_elements[:-1]):
            if isinstance(inv_element, dict):
                ...
                # The list items belong to the PREVIOUS path segment (that was
                # the key used to obtain this list). Use it as the context label.
                parent_segment = path_elements[idx - 1] if idx >= 1 else path_element
                for child_idx, i in enumerate(inv_element):
                    child_ctx = parent_context + [
                        "{}[{}]".format(parent_segment, cls._identify(i, child_idx))
                    ]
                    r = cls.match_path(
                        i, full_path, ".".join(path_elements[idx:]), child_ctx
                    )
                    results.extend(r)
                return results
            ...
            for i in inv_element:
                ...
                if isinstance(value, list):
                    ...
                    if value not in values:
                        ...
                        results.append(cls._format(full_path, value, parent_context))
                ...
                for value in list_element:
                    if value:
                        if value not in values:
                            ...
                            results.append(
                                cls._format(full_path, value, parent_context)
                            )
        return results
